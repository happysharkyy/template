<template>
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column type="selection" width="60" v-if="showBatchDelete & showOperation"></el-table-column>
      ...
      <el-table-column :label="$t('action.operation')" width="300" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          ...
          <kt-button icon="fa fa-top" :label="$t('top')" :perms="permsTop" :size="size" @click="handleTop(scope.$index, scope.row)" />
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
